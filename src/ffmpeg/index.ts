...
export const useFfmpeg = () => {
  ...
  const generateFilter = (options: VideoSettings) => {
    ...
    for (const layer of options.layer) {
      filter += `[0:v]crop=${layer.input!.rect.width}:${layer.input!.rect.height}:${layer.input!.rect.x}:${layer.input!.rect.y}[croplayer${layer.id}];`;
      filter += `[croplayer${layer.id}]scale=h=${layer.output.rect.height}:${layer.output.rect.width}[layer${layer.id}];`;
